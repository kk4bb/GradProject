...
namespace CampusConnect.Infrastructure.Services
{
    public class GradeService : IGradeService
    {
        ...
        {
            var enrolledStudents = await (from e in _context.Enrollments
                                          join u in _context.Users on e.StudentId equals u.Id
                                          where e.CourseId == courseId
                                          select new { StudentId = e.StudentId, User = u })
                                          .ToListAsync();
            ...
            var result = new List<GradeRecordDto>();
            foreach (var student in enrolledStudents)
            {
                if (gradeRecords.TryGetValue(student.StudentId, out var grade))
                {
                    result.Add(MapToDto(grade, student.User.FirstName, student.User.LastName, student.User.ProfilePictureUrl));
                }
                else
                {
                    result.Add(new GradeRecordDto
                    {
                        Id = 0,
                        StudentId = student.StudentId,
                        StudentName = !string.IsNullOrEmpty(student.User.FirstName) ? $"{student.User.FirstName} {student.User.LastName}" : "Unknown Student",
                        StudentAvatarUrl = student.User.ProfilePictureUrl,
                        CourseId = courseId,
                        QuizzesTotal = 0,
                        AssignmentsTotal = 0,
                        AttendanceTotal = 0,
